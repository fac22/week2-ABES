...
async function writeReviews(submission) {
  const { restaurant, postcode, review, username } = submission;
  const queryUsers = /* sql */ `
    INSERT INTO
      users(username)
    VALUES ($1)
  `;
  await db.query(queryUsers, [username]);
  const queryPlace = /* sql */ `
    INSERT INTO
        place
        (place_name, postcode)
    VALUES
        ($1, $2)
        `;
  await db.query(queryPlace, [restaurant, postcode]);
  const queryReviews = /* sql */ `
    INSERT INTO
      reviews(review, author_id, place_id)
    VALUES
      ($1, (SELECT author_id FROM users WHERE username = $2), (SELECT place_id FROM place WHERE place_name = $3))
  `;
  await db.query(queryReviews, [review, username, restaurant]);
  return null;
}
